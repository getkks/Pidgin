...
public static partial class Parser<TToken> {
	...
	public static Parser<TToken, IEnumerable<T>> Sequence<T>( params Parser<TToken, T>[] parsers ) => Sequence(parsers.AsEnumerable());
	...
	public static Parser<TToken, IEnumerable<T>> Sequence<T>( IEnumerable<Parser<TToken, T>> parsers ) {
		...
		return parsersArray.Length is 1
			? parsersArray[0].Select(x => new[] { x }.AsEnumerable())
			: (Parser<TToken, IEnumerable<T>>) new SequenceParser<TToken, T>(parsersArray);
	}
}
...
internal static class SequenceTokenParser<TToken, TEnumerable>
	...
	public static Parser<TToken, TEnumerable> Create( TEnumerable tokens ) {
		ArgumentNullException.ThrowIfNull(_createParser);
...
internal sealed class SequenceParser<TToken, T> : Parser<TToken, IEnumerable<T>> {
	...
	public SequenceParser( Parser<TToken, T>[] parsers ) => _parsers = parsers;
	...
	public override sealed bool TryParse( ref ParseState<TToken> state, ref PooledList<Expected<TToken>> expecteds, [MaybeNullWhen(false)] out IEnumerable<T> result ) {
		...
		for( var i = 0 ; i < _parsers.Length ; i++ ) {
			if( !_parsers[i].TryParse(ref state, ref expecteds, out ts[i]!) ) {
				result = null;
				return false;
			}
		}
...
internal sealed class SequenceTokenParserFast<TToken, TEnumerable> : Parser<TToken, TEnumerable>
	...
	public override sealed bool TryParse( ref ParseState<TToken> state, ref PooledList<Expected<TToken>> expecteds, [MaybeNullWhen(false)] out TEnumerable result ) {
