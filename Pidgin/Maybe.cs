...
public readonly struct Maybe<T> : IEquatable<Maybe<T>> {
	...
	public Maybe<U> Cast<U>() => HasValue ? Maybe.Just((U) (object) _value!) : Maybe.Nothing<U>();

	/// <inheritdoc/>
	public bool Equals( Maybe<T> other ) => HasValue && other.HasValue && Equals(Value, other.Value) || !HasValue && !other.HasValue;
	...
	public Maybe<R> SelectMany<U, R>( Func<T, Maybe<U>> selector, Func<T, U, R> result ) {
		if( HasValue ) {
			...
			return mu.HasValue ? Maybe.Just(result(_value, mu._value)) : Maybe.Nothing<R>();
		}
	...
	public Maybe<T> Where( Func<T, bool> predicate ) => HasValue && predicate(_value) ? this : Maybe.Nothing<T>();
}
