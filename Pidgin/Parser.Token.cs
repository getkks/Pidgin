...
internal sealed class TokenParser<TToken> : Parser<TToken, TToken> {
	...
	public TokenParser( TToken token ) => _token = token;
	...
	public override sealed bool TryParse( ref ParseState<TToken> state, ref PooledList<Expected<TToken>> expecteds, [MaybeNullWhen(false)] out TToken result ) {
		if( state.HasCurrent ) {
...
internal sealed class PredicateTokenParser<TToken> : Parser<TToken, TToken> {
	...
	public PredicateTokenParser( Func<TToken, bool> predicate ) => _predicate = predicate;
