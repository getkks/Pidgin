...
public struct PooledList<T> : IDisposable, IList<T> {
	...
	public int IndexOf( T item ) => Count is 0 ? -1 : Array.IndexOf(_items!, item, 0, Count);
	...
	[DoesNotReturn]
	private static void ThrowInvalidOperationException() => throw CreateInvalidOperationException();
	...
	private void Init( int space ) => _items = _arrayPool.Rent(Math.Max(InitialCapacity, space));
}
