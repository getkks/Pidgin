...
public static partial class Parser {
	...
	private static int GetLetterOrDigitValue( char c ) =>
		c switch {
			var x when x is >= '0' and <= '9' => GetDigitValue(c),
			var x when x is >= 'A' and <= 'Z' => GetUpperLetterOffset(c) + 10,
			_ => GetLowerLetterOffset(c) + 10
		};

	private static long GetLetterOrDigitValueLong( char c ) =>
		c switch {
			var x when x is >= '0' and <= '9' => GetDigitValueLong(c),
			var x when x is >= 'A' and <= 'Z' => GetUpperLetterOffsetLong(c) + 10,
			_ => GetLowerLetterOffsetLong(c) + 10
		};
	...
	private struct IntChainer : IChainer<int, int> {
		...
		public void Apply( int value ) => _result = _result * _base + value;
	...
	private struct LongChainer : IChainer<long, long> {
		...
		public void Apply( long value ) => _result = _result * _base + value;
