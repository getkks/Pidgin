...
public class Result<TToken, T> {
	...
	public ParseError<TToken>? Error => Success ? throw new InvalidOperationException("Can't get the error of a successful result") : _error;
	...
	public T Value => Success ? _value : throw new InvalidOperationException("Can't get the value of an unsuccessful result");

	internal bool ConsumedInput {
		get;
	}  // for testing, innit
	...
	public Result<TToken, R> SelectMany<U, R>( Func<T, Result<TToken, U>> selector, Func<T, U, R> result ) {
		...
		return !ru.Success
			? new Result<TToken, R>(ConsumedInput || ru.ConsumedInput, ru.Error!)
			: new Result<TToken, R>(ConsumedInput || ru.ConsumedInput, result(_value, ru._value));
	}
}
