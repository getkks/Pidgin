...
internal sealed class ReturnParser<TToken, T> : Parser<TToken, T> {
	...
	public ReturnParser( T value ) => _value = value;

	public override sealed bool TryParse( ref ParseState<TToken> state, ref PooledList<Expected<TToken>> expecteds, out T result ) {
		result = _value;
		return true;
	}
}
